...
# if gpus:
#     print("Use GPU:", gpus[0].name)
# else:
#     print("Use CPU")


# -----------------------------------------------------------------------------
# Helper: frequency-averaged targets (same semantics as PyTorch calc_freq)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Loss: negative log-likelihood (same semantics as your PyTorch log_likelihood)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Main training function (TensorFlow version of train_deephalo_synthetic)
# -----------------------------------------------------------------------------

def train_deephalo_synthetic_tf(loss_name,
                                num_epochs,
                                depth,
                                reswidth,
                                batch_size,
    ...
    Y_train_freq = load_or_calc_save_freq_data_tf(train_freq_file_path, X_train, Y_train)
    Y_test_freq = load_or_calc_save_freq_data_tf(test_freq_file_path, X_test, Y_test)

    # For evaluation (no need to move to device, TF handles that)
    X_train_eval = X_train
    Y_train_eval = Y_train
    Y_train_freq_eval = Y_train_freq

    X_test_eval = X_test
    Y_test_eval = Y_test
    Y_test_freq_eval = Y_test_freq
    ...
    logs = (f"Training TF model with Depth {depth} and Width {reswidth} for up to {num_epochs} epochs.\n")
    print(logs)

    # Build model once (necessary before saving weights, etc.)
    ...
    for epoch in range(num_epochs):
        ...
        in_sample_freq_loss = float(tf.sqrt(loss_fn(in_sample_output,
                                                    Y_train_freq_eval)).numpy())
        in_sample_loss = float(tf.sqrt(loss_fn(in_sample_output,
                                               Y_train_eval)).numpy())

        out_sample_freq_loss = float(tf.sqrt(loss_fn(out_sample_output,
                                                     Y_test_freq_eval)).numpy())
        ...
        if (epoch + 1) % 50 == 0:
            logs = (f'Epoch [{epoch + 1}/{num_epochs}], Original RMSE: {in_sample_loss:.4f}, Frequency RMSE: {in_sample_freq_loss:.8f}\n')
            print(logs)
    ...
    with open(f'depth{depth}.pkl', 'wb') as f:
        pickle.dump(in_sample_loss_list, f)
    plt.figure(figsize=(6, 4))
    plt.plot(in_sample_loss_list, label='In-Sample RMSE')
    plt.title("Loss Curve")
    plt.xlabel("Epoch")
    plt.ylabel("RMSE")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'Halo-Effect_model_depth_on_error_depth{depth}.jpg')#, format='pdf', dpi=300, bbox_inches='tight')
    ...
    # You can return everything you care about
    loss_record = {
...
# -----------------------------------------------------------------------------
# Parameter counting (same formulas as PyTorch version)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Experiment driver (similar to your PyTorch script)
# -----------------------------------------------------------------------------
